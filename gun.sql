...
CREATE TABLE `gun` (
  ...
  `PERSONAL` varchar(5) NOT NULL DEFAULT 'FALSE',
...
FOR EACH ROW BEGIN
  ...
  DECLARE ID_TYPE_DISP_TEMP VARCHAR(87);
  DECLARE ID_AUTH_DISP_TEMP VARCHAR(87);
  DECLARE ID_NUM_DISP_TEMP VARCHAR(87);
  DECLARE F4473_DISP_TEMP VARCHAR(87);
  DECLARE ALIEN_DOC_TEMP VARCHAR(87);
  ...
  IF (new.ID_GUN IS NOT NULL AND new.PERSONAL = FALSE) THEN
	...
	END CASE;
    CASE WHEN NEW.DOB_DISP=old.DOB_DISP THEN 
		SET DOB_DISP_TEMP=NULL;
	ELSE 
		SET DOB_DISP_TEMP=NEW.DOB_DISP;
	END CASE;
    ...
    END CASE;
    CASE WHEN NEW.ID_TYPE_DISP=old.ID_TYPE_DISP THEN 
		SET ID_TYPE_DISP_TEMP=NULL;
	ELSE SET ID_TYPE_DISP_TEMP=NEW.ID_TYPE_DISP;
    END CASE;
    CASE WHEN NEW.ID_AUTH_DISP=old.ID_AUTH_DISP THEN 
		SET ID_AUTH_DISP_TEMP=NULL;
	ELSE 
		SET ID_AUTH_DISP_TEMP=NEW.ID_AUTH_DISP;
	END CASE;
    CASE WHEN NEW.ID_NUM_DISP=old.ID_NUM_DISP THEN 
		SET ID_NUM_DISP_TEMP=NULL;
	ELSE 
		SET ID_NUM_DISP_TEMP=NEW.ID_NUM_DISP;
    END CASE;
    CASE WHEN NEW.F4473_DISP=old.F4473_DISP THEN 
		SET F4473_DISP_TEMP=NULL;
	ELSE 
		SET F4473_DISP_TEMP=NEW.F4473_DISP;
	END CASE;
    CASE WHEN NEW.ALIEN_DOC=old.ALIEN_DOC THEN 
		SET ALIEN_DOC_TEMP=NULL;
	ELSE 
		SET ALIEN_DOC_TEMP=NEW.ALIEN_DOC;
	END CASE;
	...
	END CASE;
    INSERT INTO GUN_LOG(ID_GUN,SERIAL,MFG,IMPORTER,MODEL,CALIBER,TYPE,ANT,CR,DATE_ACQ,FNAME_ACQ,LNAME_ACQ,LIC_ACQ,ADDR1_ACQ,ADDR2_ACQ,CITY_ACQ,STATE_ACQ,ZIP_ACQ,DATE_DISP,FNAME_DISP,LNAME_DISP,LIC_DISP,DOB_DISP,ADDR1_DISP,ADDR2_DISP,CITY_DISP,STATE_DISP,ZIP_DISP,ID_TYPE_DISP,ID_AUTH_DISP,ID_NUM_DISP,F4473_DISP,ALIEN_DOC,EDIT_REASON,ACTION_LOG,TIME_LOG,CO_ACQ,CO_DISP)
		VALUES(ID_GUN_TEMP,SERIAL_TEMP,MFG_TEMP,IMPORTER_TEMP,MODEL_TEMP,CALIBER_TEMP,TYPE_TEMP,ANT_TEMP,CR_TEMP,DATE_ACQ_TEMP,FNAME_ACQ_TEMP,LNAME_ACQ_TEMP,LIC_ACQ_TEMP,ADDR1_ACQ_TEMP,ADDR2_ACQ_TEMP,CITY_ACQ_TEMP,STATE_ACQ_TEMP,ZIP_ACQ_TEMP,DATE_DISP_TEMP,FNAME_DISP_TEMP,LNAME_DISP_TEMP,LIC_DISP_TEMP,DOB_DISP_TEMP,ADDR1_DISP_TEMP,ADDR2_DISP_TEMP,CITY_DISP_TEMP,STATE_DISP_TEMP,ZIP_DISP_TEMP,ID_TYPE_DISP_TEMP,ID_AUTH_DISP_TEMP,ID_NUM_DISP_TEMP,F4473_DISP_TEMP,ALIEN_DOC_TEMP,EDIT_REASON_TEMP,ACTION_LOG_TEMP,TIME_LOG_TEMP,CO_ACQ_TEMP,CO_DISP_TEMP);
	END IF;
END;
END */;;
DELIMITER ;
...
CREATE TABLE `gun_log` (
  ...
  `ID_AUTH_DISP` varchar(87) DEFAULT NULL,
  `ID_NUM_DISP` varchar(87) DEFAULT NULL,
  `F4473_DISP` varchar(87) DEFAULT NULL,
  `ALIEN_DOC` varchar(87) DEFAULT NULL,
  `EDIT_REASON` varchar(87) DEFAULT NULL,
